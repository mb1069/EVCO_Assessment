...
class SnakePlayer(list):
    ...
    def get_up_location(self, distance):
        tile = [self.body[0][0], self.body[0][1]]
        tile[0]-=distance
        return tile

    def get_down_location(self, distance):
        tile = [self.body[0][0], self.body[0][1]]
        tile[0]+=distance
        return tile
    ...
    def sense_danger_up(self):
        tile = self.get_up_location(1)
        return self.is_tile_dangerous(tile)    

    def sense_danger_2_up(self):
        tile = self.get_up_location(2)
        return self.is_tile_dangerous(tile)    
    
    def sense_danger_down(self):
        tile = self.get_down_location(1)
        return self.is_tile_dangerous(tile)    

    def sense_danger_2_down(self):
        tile = self.get_down_location(2)
        return self.is_tile_dangerous(tile)  
    ...
    def sense_wall_2_down(self):
        tile = self.get_down_location(2)
        return( tile[0] == 0 or tile[0] == (YSIZE-1) or tile[1] == 0 or tile[1] == (XSIZE-1) )
    
    def sense_wall_2_up(self):
        tile = self.get_up_location(2)
        return( tile[0] == 0 or tile[0] == (YSIZE-1) or tile[1] == 0 or tile[1] == (XSIZE-1) )
    ...
    while len(food) < NFOOD:
        ...
        for x in range(1, XSIZE - 1):
            for y in range(1, YSIZE - 1 ):
                if [y, x] not in snake.body and [x,y] not in food:
...
def runGame(individual):
    ...
    for run in range(0,NUM_EVALS):
        ...
        tours = 0
        steps = 0 
        while not snake.snake_has_collided() and not timer == XSIZE * YSIZE and len(snake.body)!=(XSIZE-2)*(YSIZE-2):
    ...
    # return coverage,/
    return (coverage) + avg_steps + (avg_score if avg_score>3 else 0),
...
def displayStrategyRun(individual):
    ...
    collided = False
    tmp = []
    while not collided and not timer == ((2 * XSIZE) * YSIZE) and len(snake.body)!=(XSIZE-2)*(YSIZE-2):
        # Set up the display
        win.border(0)
...
# Necessary for fully functioning solutions
    ...
    pset.addPrimitive(snake.if_wall_down, 2, name="if_wall_down")
    pset.addPrimitive(snake.if_wall_left, 2, name="if_wall_left")
    pset.addPrimitive(snake.if_wall_up, 2, name="if_wall_up")
    ...
    # displayStrategyRun("if_moving_right(if_wall_right(go_up,go_straight),if_moving_down(if_wall_down(go_right,go_straight),if_moving_left(if_wall_2_left(if_wall_up(go_straight,go_up),if_wall_left(go_down,go_straight)),if_wall_right(go_left,go_right))))")
    ...
    print runGame("if_moving_right(if_wall_right(go_up,go_straight),if_moving_down(if_wall_down(go_right,go_straight),if_moving_left(if_wall_2_left(if_wall_up(go_straight,go_up),if_wall_left(go_down,go_straight)),if_wall_right(go_left,go_right))))")
    ...
    try:
        ...
        #     pop, toolbox, int(POP*0.1), int(POP*0.9), cxpb, mutpb, GENERATIONS, stats=mstats, halloffame=hof, verbose=True)
...
if __name__ == "__main__":
    ...
    try:
        for x in range(0,iterations):
            ...
            fd = open('approach4_greedy_results.csv', 'a')
